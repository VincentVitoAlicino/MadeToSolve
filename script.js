...
async function main() {
    ...
    if (document.getElementById("Stats")) {
        //alert('Test')
        Print();
        if (localStorage.getItem('Ended')==='No'){
        SubmitResults();
        localStorage.setItem('Ended','Yes')
        alert('Ended!')
        }
        DownloadResults();
...
function ending() {
    ...
    document.getElementById("Question").textContent = 'Creative-Type Test Completed'
    document.getElementById("OptionHeading").textContent = ''
...
function Print() {
    ...
    console.log('You are a: \t\t' + localStorage.getItem('YourResult'));
...
function CreateResult() {
    ...
    for (i = 0; i < PointList.length; i++) {
        Stats += (CreativeTypes[i] + ':     \t\t\t' + PointList[i] + '%\n')
    }
...
function Rank() {
    ...
    for (let i = 0; i < PointList.length; i++) {
       PointList[i] = (Math.round((PointList[i] / TotalPoints) * 100 * 100) / 100)
    }

    localStorage.setItem('PointList', JSON.stringify(PointList))
    localStorage.setItem('CreativeTypes',JSON.stringify(CreativeTypes))

    for (let i = 0; i < PointList.length; i++) {
        Results.push(CreativeTypes[i] + ': ' + PointList[i] +'%')
    }
...
function SubmitResults() {
    ...
    // 1. Create an object with your data
    const rowData =
        [
            localStorage.getItem('Name'),
            localStorage.getItem('Surname'),
            localStorage.getItem('Email'), ...results, ...answers]


    //const rowData = fields.map(field => "1" + (localStorage.getItem(field) || ""));
    console.log(rowData)

    fetch("https://script.google.com/macros/s/AKfycbw50gxzBfWtKGZaBfVYQgVNBbdbOz-JU62KPJwX7UwfUISlPDF-Izap8OdCSHMg8fY2/exec", {
        method: "POST",
        // 2. Convert the object to a JSON string before sending
...
function DownloadResults(){
    ...
    const creativetypes = JSON.parse(localStorage.getItem("CreativeTypes") || "[]");
    //console.log(creativetypes.length)
    let csv = "Creative-Type,Percentage\n";
    for (let i = 0; i < creativetypes.length; i++){
        //console.log(creativetypes[i])
        //console.log(pointlist)
        csv += creativetypes[i] + "," + pointlist[i] +'%\n'
    }
    ...
    link.download = `Results_${localStorage.getItem('Name')}_${
            localStorage.getItem('Surname')}.csv`;
